...
## Прогресс агента
- Текущий этап: 2) v0.2 — “удобство системы” — в разработке. — агент
- Добавлена вкладка лога событий (Host/Player) с подгрузкой истории и realtime-обновлением. — агент
- Добавлена панель заявок в Player UI с принятием заявок через API. — агент
- Добавлены каркас прикладных сервисов, хранилища JSON и состояние кампании (доменные модели + тесты). — агент
- Добавлен каркас сервера (FastAPI) с pairing PIN, REST API, WebSocket рассылкой событий, экспортом/импортом кампании и логом событий. — агент
- Добавлены сервисы и API для контактов, заявок в друзья и чата с сообщениями (включая ссылки). — агент
- Добавлены структурированные ссылки в чат-сообщениях (нормализация, сериализация, события), подготовка для карточек. — агент
- Добавлены клиентские модули подписки на WebSocket события и обновления локальной модели чата для Host/Player UI. — агент
- Добавлены UI-формы чата для Host/Player с выбором ссылок и API-эндпоинты для linkables. — агент
- Добавлен Host UI раздел “Контакты/Чат” с формами контактов, friend-request и отправки сообщений. — агент
- Добавлены автологирование снятия экипировки при смене/удалении предметов и проверка открытия чата перед отправкой сообщения игроком. — агент
- Добавлены Host/Player UI для листа персонажа, инвентаря и экипировки с обновлением по WebSocket. — агент
- Добавлены вкладки квестов/способностей, управление квестами и системные сообщения с аудио и WebSocket-обновлениями. — агент
- Добавлены события для валют/ресурсов/репутаций/способностей, обработчики UI и API для записи и рассылки. — агент
- Добавлена проверка дубликатов заявок в друзья и открытого чата перед созданием заявки. — агент
- Унифицирован формат ссылок в чат-сообщениях между доменом, API, сервисами и JSON-хранилищем. — агент
- Исправлено сериализованное представление ссылок чата, чтобы не сохранять пустые поля в событиях. — агент
- Добавлены README и Makefile с инструкциями и командами для удобного локального запуска. — агент
- Добавлен UI редактора правил кампании (XP и очки статов) с API обновления и событием settings.updated. — агент
- Добавлен редактор бонусов классов за уровень с API обновления и событием class.per_level_bonus.updated. — агент
- Добавлена библиотека предметов в Host UI с созданием шаблонов и быстрым добавлением в инвентарь, а также событие item.template.upserted. — агент
- Добавлена библиотека шаблонов сообщений с быстрым отправлением в Host UI и событием message.template.upserted. — агент
- Добавлены настройки структуры и видимости секций листа персонажа с сохранением в настройках кампании и синхронизацией в Host/Player UI. — агент
- Исправлена обработка порядка секций листа персонажа при сохранении настроек (защита от пустых/некорректных значений). — агент
- Добавлены нормализация и сброс к умолчаниям для секций листа персонажа, а также кнопки быстрой перестановки в Host UI. — агент
- Добавлены вкладки чата в Host/Player UI с контактами, диалогами, отправкой сообщений и ссылками. — агент
