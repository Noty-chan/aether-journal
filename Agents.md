...
- Добавлена проверка дубликатов заявок в друзья и открытого чата перед созданием заявки. Нужно продолжать отмечать прогресс в этом файле. — агент
- Унифицирован формат ссылок в чат-сообщениях между доменом, API, сервисами и JSON-хранилищем. Нужно продолжать отмечать прогресс в этом файле. — агент
- Исправлено сериализованное представление ссылок чата, чтобы не сохранять пустые поля в событиях. Нужно продолжать отмечать прогресс в этом файле. — агент
- Добавлены README и Makefile с инструкциями и командами для удобного локального запуска. Нужно продолжать отмечать прогресс в этом файле. — агент
- Добавлен UI редактора правил кампании (XP и очки статов) с API обновления и событием settings.updated. Нужно продолжать отмечать прогресс в этом файле. — агент
- Добавлен редактор бонусов классов за уровень с API обновления и событием class.per_level_bonus.updated. Нужно продолжать отмечать прогресс в этом файле. — агент
- Добавлена библиотека предметов в Host UI с созданием шаблонов и быстрым добавлением в инвентарь, а также событие item.template.upserted. Нужно продолжать отмечать прогресс в этом файле. — агент
- Добавлена библиотека шаблонов сообщений с быстрым отправлением в Host UI и событием message.template.upserted. Нужно продолжать отмечать прогресс в этом файле. — агент
- Добавлены настройки структуры и видимости секций листа персонажа с сохранением в настройках кампании и синхронизацией в Host/Player UI. Нужно продолжать отмечать прогресс в этом файле. — агент
- Исправлена обработка порядка секций листа персонажа при сохранении настроек (защита от пустых/некорректных значений). Нужно продолжать отмечать прогресс в этом файле. — агент
- Добавлены нормализация и сброс к умолчаниям для секций листа персонажа, а также кнопки быстрой перестановки в Host UI. Нужно продолжать отмечать прогресс в этом файле. — агент
- Добавлены вкладки чата в Host/Player UI с контактами, диалогами, отправкой сообщений и ссылками. Нужно продолжать отмечать прогресс в этом файле. — агент
